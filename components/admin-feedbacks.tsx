  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-background">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-secondary/50 text-muted-foreground">
                    <tr>
                      <th className="px-6 py-4 font-medium">মুফতির নাম</th>
                      <th className="px-6 py-4 font-medium">তারিখ</th>
                      <th className="px-6 py-4 font-medium text-center">মতামত (Status)</th>
      ...
      <Dialog open={!!selectedFeedback} onOpenChange={(open) => !open && setSelectedFeedback(null)}>
        <DialogContent className="max-w-[95vw] sm:max-w-[90vw] md:max-w-5xl lg:max-w-6xl xl:max-w-7xl w-full max-h-[90vh] flex flex-col p-6 overflow-hidden">
          <DialogHeader className="shrink-0 mb-4">
            <DialogTitle className="text-xl">মতামতের বিস্তারিত (Feedback Details)</DialogTitle>
            <DialogDescription>
          ...
          <div className="flex-1 overflow-y-auto space-y-6 pr-2">
            ...
            {selectedFeedback?.user_query && (
              <div className="space-y-2">
                <h3 className="text-sm font-semibold text-primary uppercase tracking-wide border-b pb-1">User Query (প্রশ্ন)</h3>
                <div className="bg-muted/30 p-4 rounded-xl text-sm leading-relaxed text-foreground font-medium">
                  {selectedFeedback.user_query}
                </div>
              </div>
            )}
            ...
            {selectedFeedback?.ai_response?.answer && (
              <div className="space-y-2">
                <h3 className="text-sm font-semibold text-primary uppercase tracking-wide border-b pb-1">AI Answer (উত্তর)</h3>
                <div className="bg-card border p-5 rounded-xl answer-prose text-[0.95rem] leading-relaxed text-foreground text-justify" dir="auto">
            ...
            {/* Mufti Feedback */}
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-primary uppercase tracking-wide border-b pb-1 flex items-center gap-2">
              ...
              {selectedFeedback?.feedback_text ? (
                <div
                  className="bg-accent/20 p-4 rounded-xl text-sm leading-relaxed text-foreground prose prose-sm max-w-none dark:prose-invert"
                  dangerouslySetInnerHTML={{ __html: selectedFeedback.feedback_text }}
                />
              ) : (
                <div className="bg-muted/10 p-4 rounded-xl text-sm italic text-muted-foreground border border-dashed">
                  কোনো টেক্সট মতামত দেওয়া হয়নি।
                </div>
              )}
            </div>
          </div>
      ...
      {feedbackToDelete && (
        <ConfirmDialog
          ...
          description="আপনি কি নিশ্চিত যে আপনি এই মতামতটি মুছে ফেলতে চান? এই অ্যাকশনটি পূর্বাবস্থায় ফেরানো যাবে না।"
